...
const Chat = ({
  ...
  return (
    ...
    >
      ...
      {/* HEADER */}
      <div
        className={`border-b flex items-center justify-between relative z-20 shrink-0 select-none transition-all duration-300 ${
          isHeaderCollapsed ? "p-2 sm:p-2.5 px-4 sm:px-5" : "p-3 sm:p-4 2xl:p-5"
        } ${
          darkMode
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              {onBack && (
                <button
                  ...
                  className={`lg:hidden p-1.5 rounded-lg transition-all border shrink-0 ${
                    darkMode
              ...
              <Hash className={`w-3.5 h-3.5 shrink-0 ${darkMode ? "text-blue-400" : "text-blue-600"}`} />
              <span className={`font-bold text-xs sm:text-sm ${darkMode ? "text-blue-300" : "text-blue-600"}`}>
                #{ticket?.ticketId || "N/A"}
              </span>
              <span className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"} truncate max-w-[120px] sm:max-w-xs`}>
                ({ticket?.customerName || "Customer"})
              </span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                ticket?.status === "OPEN" ? "bg-amber-500/10 text-amber-500" :
                ticket?.status === "ASSIGNED" ? "bg-blue-500/10 text-blue-500" :
                ticket?.status === "IN_PROGRESS" ? "bg-indigo-500/10 text-indigo-500" :
