...
const StatusBadge = ({ status, isDark }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border flex-shrink-0 ${getStatusClass(status, isDark)}`}
    >
                ...
                <>
                  <td className="py-3 px-4 align-top">
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span
                        className={`truncate font-mono text-sm font-medium flex-shrink-0 ${isDark ? "text-white" : "text-gray-900"}`}
                      >
                  ...
                  <td className="py-3 px-4 align-top">
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span
                        className={`truncate text-sm flex-shrink-0 ${isDark ? "text-gray-300" : "text-gray-700"}`}
                      >
                  ...
                  <td className="py-3 px-4 align-top">
                    <div className="truncate text-sm max-w-[200px]" title={ticket.subject}>
                      {ticket.subject}
                    </div>
                  </td>
                  <td className="py-3 px-4 align-top">
                  ...
                  <td className="py-3 px-4 align-top">
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span
                        className={`text-sm flex-shrink-0 ${isDark ? "text-gray-400" : "text-gray-600"}`}
                      >
            ...
            <DataTable
              ...
              renderRow={(payment) => (
                <>
                  ...
                  <td className="py-3 px-4 align-top">
                    <div className="min-w-0 flex flex-col flex-shrink-0">
                      <span
                        className={`truncate text-sm font-medium flex-shrink-0 ${isDark ? "text-gray-200" : "text-gray-800"}`}
                      >
                        {payment.planName || payment?.plan?.planName || "--"}
                      </span>
                      {payment.profileId && (
                        <span className="truncate text-xs text-gray-500 flex-shrink-0">
                          {payment.profileId}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="py-3 px-4 align-top">
                    <span
                      className={`text-sm font-semibold flex-shrink-0 ${isDark ? "text-white" : "text-gray-900"}`}
                    >
                      {formatCurrency(
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div
          className={`mt-4 h-1 rounded-full overflow-hidden ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
        >
          <div
...
const SectionCard = ({ title, icon, children, isDark }) => (
  ...
  >
    <div
      className={`px-4 py-4 sm:px-6 sm:py-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}
    >
      ...
      <table className="w-full min-w-[760px] table-auto">
        ...
        <tbody>
          {data.slice(0, 5).map((item, idx) => (
            <tr
              key={
                item.ticketId || item.paymentId || item.razorpayPaymentId || idx
              }
...
const SkeletonCards = () => (
  <>
    {[1, 2, 3, 4].map((i) => (
      <div
        key={i}
        className="h-40 rounded-2xl bg-gradient-to-br from-gray-200/40 to-gray-300/20 dark:from-gray-700/40 dark:to-gray-800/30 animate-pulse"
      />
    ))}
  </>
);
