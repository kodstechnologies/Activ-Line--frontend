...
const SubscribersPage = () => {
  ...
  const handleGoToPlans = () => {
    ...
    const createdPassword =
      createdCustomer?.password || createdCustomer?.data?.password || "";
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const createdPayload =
          res?.data?.data || res?.data?.customer || res?.data || null;
        setCreatedCustomer(createdPayload);
        setPaymentStatus({
          type: "success",
          message: "Customer created. You can select plans now.",
        });
        saveDraft(newSubscriber, files, paymentForm, paymentVerified);
      }
    } catch (err) {
  ...
  const handleEditClick = (subscriber, e) => {
    ...
    setEditSubscriber({
      ...
      customActivationDate:
        formatDateForInput(subscriber.customActivationDate) || "",
      customExpirationDate:
        formatDateForInput(subscriber.customExpirationDate) || "",
      ...
      installation_address_country:
        subscriber.installationAddress?.country || "IN",
      caf_num: subscriber.cafNum || "",
      createBilling:
        subscriber.createBilling === "on" || subscriber.createBilling === true,
      notifyUserSms:
        subscriber.notifyUserSms === "on" || subscriber.notifyUserSms === true,
  ...
  const handleUpdateSubscriber = async () => {
    ...
    try {
      ...
      const updateId =
        selectedSubscriber.activlineUserId || selectedSubscriber.id;
    ...
    async (page, limit) => {
      try {
        ...
        const params = {
          ...
          accountId:
            selectedFranchise !== "All" ? selectedFranchise : undefined,
        };
        ...
        setTotalItems(
          Number(
            pagination.total ?? pagination.totalItems ?? customers.length ?? 0,
          ),
        );
        setTotalPages(
  ...
  useEffect(() => {
    if (
      ...
      setNewSubscriber((prev) => ({
        ...prev,
        userGroupId: paymentForm.groupId,
      }));
    }
  ...
  return (
    <div
      className={`rounded-xl shadow-sm border flex flex-col h-full relative ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
    >
      <div
        className={`p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${isDark ? "border-slate-800" : "border-gray-200"}`}
      >
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 w-full lg:w-auto">
          {canEditCustomer && (
            <button
              ...
              className="flex items-center justify-center w-full sm:w-auto px-4 py-2 rounded-lg text-sm font-semibold text-white bg-blue-600 hover:bg-blue-500 shadow-sm whitespace-nowrap"
            >
              Add Customer
            </button>
          )}
          ...
          {/* Search */}
          <div className="relative w-full sm:w-72 lg:w-72">
            <Search
        ...
        isDark
          ...
          <div className="relative w-full sm:w-auto">
            <button
              ...
              className={`flex items-center justify-center w-full px-3 py-2 border rounded-lg whitespace-nowrap
      ${
        isDark
          ...
          : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"
            ...
            {showFilter && (
              <div
                className={`absolute left-1/2 -translate-x-1/2 sm:left-auto sm:right-0 sm:translate-x-0 top-12 w-[calc(100vw-32px)] sm:w-80 p-4 rounded-xl shadow-2xl border z-50 animate-in fade-in zoom-in duration-150
        ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-gray-200"}`}
              >
                ...
                <label
                  className={`text-sm font-semibold ${isDark ? "text-slate-400" : "text-gray-600"}`}
                >
                  Status
                </label>
                <select
                ...
                {franchiseError && (
                  <div
                    className={`mt-3 text-xs ${isDark ? "text-yellow-300" : "text-yellow-700"}`}
                  >
                    {franchiseError}
                  </div>
                )}
...
/>
            ...
            <div className="overflow-x-auto overflow-y-auto h-full">
              <table className="w-full text-left">
                <thead>
                  ...
                  >
                    <th
                      className={`py-4 px-6 text-sm font-semibold uppercase tracking-wider ${isDark ? "text-slate-400" : "text-gray-600"}`}
                    >
                      Customer
                    </th>
                    <th
                    ...
                    <th
                      className={`py-4 px-6 text-sm font-semibold uppercase tracking-wider text-right ${isDark ? "text-slate-400" : "text-gray-600"}`}
                    >
                      Actions
                    </th>
                  </tr>
                ...
                >
                  {loading ? (
                    Array.from({ length: Math.min(itemsPerPage, 8) }).map(
                      (_, i) => (
                        <tr
                          key={`skeleton-${i}`}
                          className={`${isDark ? "bg-slate-900/30" : "bg-white"} animate-pulse`}
                        >
                          <td className="py-5 px-6">
                            <div className="flex items-center gap-4">
                              <div
                                className={`w-11 h-11 rounded-full ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                              <div className="space-y-2 min-w-[180px]">
                                <div
                                  className={`h-4 w-36 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                                />
                                <div
                                  className={`h-3 w-24 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                                />
                              </div>
                            </div>
                          </td>
                          <td className="py-5 px-6">
                            <div className="space-y-2">
                              <div
                                className={`h-4 w-28 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                              <div
                                className={`h-3 w-40 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                            </div>
                          </td>
                          <td className="py-5 px-6">
                            <div className="space-y-2">
                              <div
                                className={`h-4 w-24 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                              <div
                                className={`h-3 w-16 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                            </div>
                          </td>
                          <td className="py-5 px-6">
                            <div className="space-y-2">
                              ...
                              />
                              <div
                                className={`h-3 w-20 rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                            </div>
                          </td>
                          <td className="py-5 px-6">
                            <div
                              className={`h-7 w-24 rounded-full ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                            />
                          </td>
                          <td className="py-5 px-6">
                            <div className="flex items-center justify-end gap-2">
                              <div
                                className={`h-8 w-8 rounded-lg ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                              <div
                                className={`h-8 w-8 rounded-lg ${isDark ? "bg-slate-700" : "bg-gray-200"}`}
                              />
                    ...
                    paginationData.paginatedSubscribers.map((sub, i) => (
                      ...
                      >
                        <td className="py-5 px-6">
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-11 h-11 rounded-full bg-blue-500/10 text-blue-400 flex items-center justify-center font-bold text-base flex-shrink-0 ${isDark ? "ring-2 ring-blue-500/20" : "ring-2 ring-blue-500/10"}`}
                            >
                              {sub.name.charAt(0)}
                            </div>
                            <div className="min-w-0">
                              ...
                              <div
                                className={`text-sm mt-0.5 ${isDark ? "text-slate-500" : "text-gray-500"}`}
                              >
                                {sub.userName || "N/A"}
                              </div>
                            </div>
                        ...
                        <td className="py-5 px-6">
                          <div
                            className={`text-base font-medium ${isDark ? "text-slate-200" : "text-gray-700"}`}
                          >
                            {sub.mobile || "N/A"}
                          </div>
                          <div
                        ...
                        <td className="py-5 px-6">
                          <div
                            className={`text-base font-medium ${isDark ? "text-slate-200" : "text-gray-700"}`}
                          >
                            {franchiseLookup.get(sub.accountId) ||
                              sub.accountId ||
                              "N/A"}
                          </div>
                          <div
                    ...
                    <tr>
                      <td
                        colSpan="5"
                        className={`py-12 text-center text-base ${isDark ? "text-slate-400" : "text-gray-500"}`}
                      >
                ...
                .map((page, index, array) => {
                  ...
                  return (
                    <React.Fragment key={page}>
                      {showEllipsisBefore && (
                        <span
                          className={`px-2 text-base ${isDark ? "text-slate-500" : "text-gray-500"}`}
                        >
                          ...
                        </span>
                      )}
      ...
      {isModalOpen &&
        createPortal(
          <div className=" fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <div
                className={`p-4 border-b flex justify-between items-center rounded-t-xl ${isDark ? "border-slate-800 bg-slate-900" : "border-gray-200 bg-white"}`}
              >
                <h3
                ...
                <button
                  ...
                  className={`transition-colors ${isDark ? "text-slate-400 hover:text-white" : "text-gray-400 hover:text-gray-600"}`}
                >
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Personal Info */}
                <h4
                  className={`text-base font-bold uppercase mt-4 ${isDark ? "text-slate-500" : "text-gray-500"}`}
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label
                      className={`block text-base font-medium mb-1 ${isDark ? "text-slate-400" : "text-gray-600"}`}
                    >
                      First Name
                    </label>
                    <input
                ...
                <div className="flex gap-6 mt-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span
                      className={`text-base ${isDark ? "text-slate-300" : "text-gray-700"}`}
                    >
                      Create Billing
                    </span>
                  </label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {["idFile", "addressFile", "signFile", "profilePicFile"].map(
                    (fileKey) => (
                      <div key={fileKey}>
                        <label
                          className={`block text-base font-medium mb-1 capitalize ${isDark ? "text-slate-400" : "text-gray-600"}`}
                        >
                          {fileKey
                            .replace("File", "")
                            .replace(/([A-Z])/g, " $1")}
                ...
                {Object.values(files).some(Boolean) && (
                  <div
                    className={`mt-3 text-sm ${isDark ? "text-slate-300" : "text-gray-700"}`}
                  >
                ...
                {/* Franchise & Payment */}
                <h4
                  className={`text-base font-bold uppercase mt-6 ${isDark ? "text-slate-500" : "text-gray-500"}`}
                >
                  Franchise & Payment
                </h4>
                <div
                  className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${isUserDetailsComplete ? "" : "opacity-60 pointer-events-none"}`}
                >
                  <div>
                    <label
                      className={`block text-base font-medium mb-1 ${isDark ? "text-slate-400" : "text-gray-600"}`}
                  ...
                  {isUserDetailsComplete &&
                    ...
                    !createdCustomer && (
                      <div
                        className={`text-sm ${isDark ? "text-slate-400" : "text-gray-600"}`}
                      >
                        Create the customer first. After creation, you can
                        select plans and pay.
                      </div>
                    )}
                </div>
              ...
              <div
                className={`p-4 border-t flex gap-3 justify-end rounded-b-xl ${isDark ? "border-slate-800 bg-slate-900" : "border-gray-200 bg-white"}`}
              >
                <button
                  onClick={handleCancelAddSubscriber}
                  className={`px-4 py-2 text-base font-medium rounded-lg transition-colors ${isDark ? "text-slate-300 hover:bg-slate-800" : "text-gray-700 hover:bg-gray-100"}`}
                >
                  Cancel
                </button>
                <button
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 space-y-4">
                {/* Account Info */}
                <h4
                  className={`text-base font-bold uppercase ${isDark ? "text-slate-500" : "text-gray-500"}`}
                >
                  Account Information
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      name="userGroupId"
                      value={editSubscriber.userGroupId}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="firstName"
                      value={editSubscriber.firstName}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="tel"
                      name="phoneNumber"
                      value={editSubscriber.phoneNumber}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      name="activationDate"
                      value={editSubscriber.activationDate}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    >
                      <option value="now">Now</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <input
                      type="text"
                      name="installation_address_line2"
                      value={editSubscriber.installation_address_line2}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                      placeholder="Floor, Building, Street"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="installation_address_city"
                      value={editSubscriber.installation_address_city}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="installation_address_pin"
                      value={editSubscriber.installation_address_pin}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      name="installation_address_country"
                      value={editSubscriber.installation_address_country}
                      onChange={handleEditInputChange}
                      className={`w-full p-2.5 border rounded-lg text-base outline-none focus:border-blue-500 ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-gray-50 border-gray-300 text-gray-900"}`}
                    />
                  </div>
                ...
                <div className="flex gap-6 mt-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      name="createBilling"
                      checked={editSubscriber.createBilling}
                      onChange={handleEditInputChange}
                      className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      name="notifyUserSms"
                      checked={editSubscriber.notifyUserSms}
                      onChange={handleEditInputChange}
                      className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span
                ...
                {selectedSubscriber?.documents && (
                  <div className="mb-4 space-y-3">
                    {[
                      ...
                      [
                        "Address Proof",
                        selectedSubscriber.documents.addressFile,
                      ],
                      ...
                      [
                        "Profile Photo",
                        selectedSubscriber.documents.profilePicFile,
                      ],
                    ]
              ...
              >
                ...
                <button
                  ...
                  disabled={!editSubscriber.firstName}
                  className="px-4 py-2 text-base font-bold text-white bg-green-600 rounded-lg hover:bg-green-500 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  Update Customer
                </button>
              </div>
      ...
      {isViewModalOpen &&
        selectedSubscriber &&
        createPortal(
          <div className="scrollbar-hide  fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div
              className={`scrollbar-hide rounded-xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in duration-200 border ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
            >
              <div
                className={`p-4 border-b flex justify-between items-center rounded-t-xl ${isDark ? "border-slate-800 bg-slate-900" : "border-gray-200 bg-white"}`}
              >
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 flex items-center gap-4 pb-4 border-b">
                    <div
                      className={`w-16 h-16 rounded-full bg-blue-500/10 text-blue-400 flex items-center justify-center font-bold text-xl flex-shrink-0 ${isDark ? "ring-2 ring-blue-500/20" : "ring-2 ring-blue-500/10"}`}
                    >
                  ...
                  <div className="col-span-2">
                    <label
                      className={`block text-sm font-medium mb-1 uppercase tracking-wide ${isDark ? "text-slate-500" : "text-gray-500"}`}
                    >
                      Mobile Number
                    </label>
                    <p
                  ...
                  <div className="col-span-2">
                    ...
                    <p
                      className={`text-base font-medium ${isDark ? "text-white" : "text-gray-900"}`}
                    >
                      {selectedSubscriber.name.toLowerCase().replace(" ", ".")}
                      @example.com
                    </p>
                  </div>
                  <div className="col-span-2">
                    ...
                    <p
                      className={`text-base font-medium ${isDark ? "text-white" : "text-gray-900"}`}
                    >
                      {selectedSubscriber.location}, Bangalore
                    </p>
                  </div>
                  <div>
                    <label
             ...
             bg-slate-800 text-white border-slate-700 hover:bg-slate-700"
                ...
                <button
                  onClick={() => setIsViewModalOpen(false)}
                  className="px-4 py-2 text-base font-medium rounded-lg transition-colors bg-blue-600 text-white hover:bg-blue-500"
                >
                  Close
                </button>
              </div>
