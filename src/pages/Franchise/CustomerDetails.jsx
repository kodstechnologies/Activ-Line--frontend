...
import { 
  ...
  User, FileText,
  AlertCircle, Shield, Ticket, Clock, CheckCircle, Zap, CreditCard, Award, Star, Layers, BarChart3, Download
} from 'lucide-react';
  ...
  useEffect(() => {
    const loadTickets = async () => {
      try {
        ...
        const rooms = roomsRes?.data?.rooms || roomsRes?.rooms || roomsRes?.data || [];
        ...
        const filteredRooms = (Array.isArray(rooms) ? rooms : []).filter((room) => {
          ...
          const roomUserId =
            ...
            '';
          return (
            (targetUserName && roomUserName && roomUserName === targetUserName) ||
            (roomUserId && (roomUserId === id || roomUserId === customer?.activlineUserId))
          );
        });
