...
const CustomerDetails = () => {
  ...
  const handleCreateChatRoom = async () => {
    ...
    try {
      ...
      if (newRoomId) {
        navigate("/Zone-tickets", { state: { selectedRoomId: newRoomId } });
      }
    } catch (err) {
  ...
  useEffect(() => {
    const loadTickets = async () => {
      try {
        if (!id) return;
        const roomsRes = await getTicketRooms(1, 100);
        const rooms =
          roomsRes?.data?.rooms || roomsRes?.rooms || roomsRes?.data || [];
        ...
        const filteredRooms = (Array.isArray(rooms) ? rooms : []).filter(
          (room) => {
            ...
            return (
              (targetUserName &&
                roomUserName &&
                roomUserName === targetUserName) ||
              (roomUserId &&
                (roomUserId === id || roomUserId === customer?.activlineUserId))
            );
          },
        );
        const mappedRooms = filteredRooms.map((room, idx) => ({
    ...
    loadTickets();
  }, [id, customer?.userName, customer?.activlineUserId]);
  ...
  const prefillPaymentFormFromLatestPlan = (plan) => {
    ...
    const groupId =
      groupIdRaw !== undefined && groupIdRaw !== null ? String(groupIdRaw) : "";
    const profileId =
      profileIdRaw !== undefined && profileIdRaw !== null
        ? String(profileIdRaw)
        : "";
  ...
  const startPlanPayment = async ({ groupId, profileId, amount, planType }) => {
    ...
    if (
      ...
      setPaymentStatus({
        type: "error",
        message: "Current plan details are missing (groupId/profileId/amount).",
      });
      return;
    }
    ...
    try {
      ...
      // Prefer the key returned by the backend so test/live mode always matches.
      // Fallback to the env variable only if the backend did not return one.
      const keyId =
  ...
  if (!customer) {
    return (
      <div className="space-y-6 p-6 min-h-screen">
        ...
        <div
          className={`rounded-xl shadow-sm border p-8 text-center ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
        >
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              <h1
                className={`text-4xl font-bold ${isDark ? "bg-gradient-to-r from-white via-violet-200 to-white bg-clip-text text-transparent" : "text-gray-900"}`}
              >
                {customer.firstName} {customer.lastName}
              </h1>
              <p
                className={`text-sm mt-1 ${isDark ? "text-white/60" : "text-gray-500"}`}
              >
            ...
            <div className={`${glassCardClass} p-6`}>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3 group">
                  <div
                    className={`p-2 rounded-lg transition-all ${isDark ? "bg-violet-500/20 group-hover:bg-violet-500/30" : "bg-violet-100 group-hover:bg-violet-200"}`}
                  >
                    <Mail
          ...
          <div className="space-y-6 lg:col-span-2">
            <div className={`${glassCardClass} p-6`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <h3
                  className={`text-lg font-semibold flex items-center gap-2 ${isDark ? "text-white/90" : "text-gray-900"}`}
                >
                  <Ticket
                    className={`w-5 h-5 ${isDark ? "text-violet-300" : "text-violet-500"}`}
                  />
                ...
                <div
                  className={`rounded-lg px-3 py-1.5 text-sm ${isDark ? "bg-white/5 text-white/60" : "bg-gray-100 text-gray-600"}`}
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                <div className="relative">
                  <Search
                    className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 ${isDark ? "text-white/40" : "text-gray-400"}`}
                  />
                  <input
                    type="text"
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr
                      className={`border-b ${isDark ? "border-white/10 text-white/40" : "border-gray-200 text-gray-500"} uppercase text-xs`}
                    >
                      <th className="py-3 font-semibold tracking-wider">
                        Ticket ID
                      </th>
                      <th className="py-3 font-semibold tracking-wider">
                        Date
                  ...
                  >
                    {paginatedTickets.map((ticket) => (
                      <tr
                        key={ticket.id}
                        className={`group transition-all cursor-pointer ${isDark ? "hover:bg-white/5" : "hover:bg-violet-50"}`}
                      >
                        <td
                          className={`py-4 font-mono text-xs ${isDark ? "text-white/50" : "text-gray-500"}`}
                        >
                          #{ticket.id}
                        </td>
                        <td
                          className={`py-4 ${isDark ? "text-white/70" : "text-gray-600"}`}
                        >
                        ...
                        <td
                          className={`py-4 font-medium ${isDark ? "text-violet-300" : "text-violet-600"}`}
                        >
                          {ticket.subject}
                        </td>
                        <td className="py-4">
              ...
              {filteredTickets.length > 0 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t ${isDark ? 'border-white/10' : 'border-gray-200'}">
                  <div
                    className={`text-sm ${isDark ? "text-white/50" : "text-gray-500"}`}
                  >
                    Showing {(ticketCurrentPage - 1) * ticketsPerPage + 1} to{" "}
                    {Math.min(
                      ticketCurrentPage * ticketsPerPage,
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        setTicketCurrentPage((prev) => Math.max(1, prev - 1))
                      }
                    ...
                    <div className="flex gap-1">
                      {Array.from(
                        { length: Math.min(5, totalTicketPages) },
                        (_, i) => {
                          let pageNum;
                    ...
                    <button
                      onClick={() =>
                        setTicketCurrentPage((prev) =>
                          Math.min(totalTicketPages, prev + 1),
                        )
                      }
            ...
            <div className={`${glassCardClass} p-6`}>
              ...
              {paymentsLoading ? (
                ...
                <div
                  className={`rounded-xl border p-6 text-center text-sm ${isDark ? "border-white/10 bg-white/5 text-white/60" : "border-gray-200 bg-gray-50 text-gray-500"}`}
                >
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    >
                      {customerPayments.map((payment) => (
                        <tr
                          key={payment.paymentId || payment.razorpayPaymentId}
                          className={`${isDark ? "hover:bg-white/5" : "hover:bg-violet-50"} transition-all`}
                        >
                          <td className="py-4">
                            <div>
                              <div
                          ...
                          <td
                            className={`py-4 font-semibold ${isDark ? "text-white" : "text-gray-900"}`}
                          >
                            {formatPaymentAmount(
                              payment.amount,
                              payment.currency,
        ...
        {isPlanModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
            <div
              className={`w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border ${isDark ? "bg-slate-900/95 border-white/10" : "bg-white/95 border-gray-200"} backdrop-blur-xl`}
            >
              <div
                className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? "border-white/10" : "border-gray-200"}`}
              >
                <div>
                  <h3
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
                <div className="lg:col-span-2 space-y-4">
                  {/* Groups section */}
                  <div
                    className={`rounded-xl border p-4 ${isDark ? "border-white/10 bg-black/20" : "border-gray-200 bg-gray-50"}`}
                  >
                    ...
                    <label
                      className={`block text-xs font-semibold mb-2 ${isDark ? "text-white/60" : "text-gray-600"}`}
                    >
                      Select Group
                    </label>
                    <select
                    ...
                    {selectedGroupId && (
                      <div
                        className={`mt-3 text-xs ${isDark ? "text-white/60" : "text-gray-600"}`}
                      >
                            ...
                            >
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  >
                                    <Star
                                      className={`w-3 h-3 ${isDark ? "text-amber-300" : "text-amber-500"}`}
                                    />
                                    {info.name || "Profile"}
                                  </div>
                                  <div
                                    className={`text-xs ${isDark ? "text-white/60" : "text-gray-500"}`}
                                  >
                                    Profile ID: {info.profileId || "—"} | Group
                                    ID: {info.groupId || "—"}
                                  </div>
                                  {(info.planName || info.amount) && (
                                    <div
                                      className={`text-xs mt-1 ${isDark ? "text-white/60" : "text-gray-500"}`}
                                    >
                                      {info.planName
                                        ? `Plan: ${info.planName}`
                                        : ""}
                ...
                <div>
                  <div
                    className={`rounded-xl border p-4 ${isDark ? "border-white/10 bg-black/20" : "border-gray-200 bg-white"}`}
                  >
                    <h4
                    ...
                    <div className="space-y-3 text-sm">
                      <div>
                        ...
                        <div
                          className={`font-semibold font-mono text-sm ${isDark ? "text-white" : "text-gray-900"}`}
                        >
