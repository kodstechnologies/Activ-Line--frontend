      ...
      {isOpen && details && (
        ...
        >
          ...
          >
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center gap-2 mb-2">
                        <div
                          className={`p-1.5 rounded-lg bg-gradient-to-r ${item.color}`}
                        >
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    >
                      This {details.packageType} plan offers{" "}
                      {details.monthlyPlan} with {details.connectionType}{" "}
                      connection. Billed {details.billingType.toLowerCase()} at
                      ₹{details.totalPrice} per {details.period.toLowerCase()}.
                    </p>
                  </div>
                </div>
              </motion.div>
            ...
            {/* Modal Footer */}
            <div
              className={`flex justify-end p-4 border-t ${isDark ? "border-slate-700" : "border-purple-100"}`}
            >
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={onClose}
                className={` px-16 py-3 rounded-lg font-medium transition-all duration-300 ${
                  isDark
  ...
  const handleViewDetails = async (plan) => {
    ...
    try {
      ...
      const profileDetails =
        message["profile Details"] || message.profileDetails || [];
      const billingDetails =
        message["billing Details"] || message.billingDetails || [];
          ...
          <table className="w-full text-left">
            <thead>
              <tr className={`${isDark ? "bg-slate-800/60" : "bg-gray-50/80"}`}>
                <th
                  className={`py-4 px-6 text-xs uppercase font-semibold tracking-wider ${
                ...
                <th
                  className={`py-4 px-6 text-xs uppercase font-semibold tracking-wider text-right ${
                    isDark ? "text-slate-300" : "text-gray-600"
                  }`}
                >
