...
import { getAssignedCustomersGraphSummary } from "../../../api/graphSummary.api";
...
const Reports = () => {
    ...
    useEffect(() => {
        const loadReport = async () => {
            try {
                setLoading(true);
                setError("");
                const data = await getAssignedCustomersGraphSummary({ months });
    ...
    const supportData = useMemo(() => {
        const rows = Array.isArray(summary?.monthlyResolvedTickets)
            ? summary.monthlyResolvedTickets
            : [];
        return rows.map((item) => ({
            period: formatMonthLabel(item.month),
