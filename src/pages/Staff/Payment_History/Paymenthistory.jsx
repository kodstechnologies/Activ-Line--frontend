...
const BillingPage = () => {
  ...
  const loadTransactions = useCallback(async () => {
    try {
      ...
      const pages =
        Number(
          meta?.totalPages ?? payload?.totalPages ?? payload?.pages ?? 0,
  ...
  return (
    <div className="space-y-6">
      <div
        className={`rounded-xl shadow-sm border flex flex-col h-full ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}
      >
        <div
          className={`p-6 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-4 relative ${isDark ? "border-slate-800" : "border-gray-200"}`}
        >
          <div>
            <h1
            ...
            >
              Track assigned customer payments
            </p>
          </div>
