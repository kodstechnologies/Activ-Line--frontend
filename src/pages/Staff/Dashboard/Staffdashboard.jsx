...
const DashboardHeaderAnimation = memo(({ isDark }) => (
  <div
    className={`flex items-center justify-center rounded-xl p-2 ${isDark ? "bg-blue-500/10" : "bg-blue-50"}`}
  >
    <Lottie
...
const StatusIcon = memo(({ status }) => {
  ...
  const { icon: Icon, color } = STATUS_CONFIG[key] || {
    icon: AlertCircle,
    color: "gray",
  };
  const colorClass =
...
const DashboardCard = memo(
  ({ title, value, icon: Icon, color, bgColor, textColor, isDark, index }) => (
    <div
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br ${color} opacity-0 hover:opacity-5 transition-opacity`}
      />
...
const SectionCard = memo(({ title, subtitle, icon, children, isDark }) => (
  <div
    className={`rounded-2xl overflow-hidden ${isDark ? "bg-gray-800/70 border border-gray-700" : "bg-white shadow-md"}`}
  >
    <div
      className={`px-6 py-4 border-b flex items-center gap-3 ${isDark ? "border-gray-700" : "border-gray-200"}`}
    >
      <div
...
const EmptyState = memo(({ type, isDark }) => {
  ...
  return (
    <div
      className={`p-12 text-center ${isDark ? "bg-gray-800/40" : "bg-gray-50"} rounded-xl`}
    >
...
const TicketRow = memo(({ item, isDark, navigate }) => {
  ...
  return (
    <tr
      className={`border-b last:border-0 hover:bg-opacity-60 transition-colors ${
        isDark
          ? "hover:bg-gray-700/60 border-gray-700/60"
    ...
    >
      <td className="py-4 px-4">
        <div className="flex items-center gap-3">
          <div
            className={`p-2.5 rounded-lg ${isDark ? "bg-gray-700" : "bg-gray-100"}`}
          >
          ...
          <div>
            ...
            <div
              className={`text-xs mt-0.5 ${isDark ? "text-gray-400" : "text-gray-900"}`}
            >
              {customer}
            </div>
          </div>
      ...
      <td
        className={`py-4 px-4 text-sm truncate max-w-xs ${isDark ? "text-gray-300" : "text-gray-900"}`}
        title={customer}
      >
        {customer}
      </td>
      ...
      <td
        className={`py-4 px-4 text-sm ${isDark ? "text-gray-400" : "text-gray-900"}`}
      >
...
const TicketsTable = memo(({ tickets, isDark, navigate }) => {
  ...
  return (
    <>
      ...
      <div
        className={`mt-5 text-center ${isDark ? "border-t border-gray-700 pt-4" : "border-t border-gray-200 pt-4"}`}
      >
        <button
          onClick={() => navigate("/assigned-tickets")}
...
const DashboardPage = () => {
  ...
  const loadDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const [open, inProg, today, total, ticketsRes, paymentsRes] =
        await Promise.all([
          getOpenTickets(),
          getInProgressTickets(),
          getTodayResolvedTickets(),
        ...
        {loading
          ? Array(4)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="rounded-2xl p-6 bg-gray-200/60 dark:bg-gray-800/50 animate-pulse h-40"
                />
              ))
          : cardItems.map((card) => (
              <DashboardCard key={card.title} {...card} isDark={isDark} />
            ))}
      </div>
            ...
            <div className="space-y-4 py-2">
              {Array(5)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="h-16 bg-gray-200 dark:bg-gray-700/50 rounded-xl animate-pulse"
                  />
                ))}
            </div>
          ) : (
